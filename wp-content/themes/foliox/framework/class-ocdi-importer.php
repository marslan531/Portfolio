	...
	public function import_files_config() {
		...
		$home_prevs = array(
			'medinet_home_1' => array(
				'title' => __( 'Import Foliox', 'foliox' ),
				'page'  => __( 'Home', 'foliox' ),
				'screenshot' => get_template_directory_uri() . '/sample-data/screenshot.png',
				'preview_link' => 'https://shtheme.com/demosd/foliox/',
			),
		);
		...
		foreach ( $home_prevs as $key => $prev ) {
			...
			$config[] = array(
				...
				'import_notice'              => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'foliox' ),
			);
		}
